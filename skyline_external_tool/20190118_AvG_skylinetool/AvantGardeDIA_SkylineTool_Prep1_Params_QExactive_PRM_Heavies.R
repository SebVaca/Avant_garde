...
KeepPeptidesWithLowerNumberOfTransitions=TRUE # Keep (or not) a peptide if it has a lower number of transitions than the limit above (recomended FALSE for DIA). The transition refinement refinement will not be done on these peptides but the peak boundary refienement and scoring will be performed.
...
UseHeavyPeakBoundariesForLight=TRUE # Use the heavy peptide peak boundaries for the light peptides.
